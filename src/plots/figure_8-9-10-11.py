...
model_experiment = {
    'LSTM': [['exp01_lstm', 'exp02_lstm', 'exp03_lstm', 'exp04_lstm', 'exp05_lstm',
             ...
             'average f1 improvement (%)', 'average time increase (%)'
             ],
...
for model in model_experiment:
    ...
    ax.plot(est[1:], linewidth=8, color=color_f1, alpha=1)

    ax.set_xticklabels(model_experiment[model][1])
    ax.tick_params(direction='out', length=8, width=3, color='darkgray')
    ax.set_xlabel(model_experiment[model][2], size=16, weight='regular')
    ...
    plt.xticks(np.arange(0, len(model_experiment[model][1]), 1))
    ...
    ax.plot(est[1:], linewidth=8, color=color_tm, alpha=1, linestyle='--', marker='o', markersize=15)
    for i, value in enumerate(est[1:]):
        text = str(int(value * v)) + '%'
        ax.annotate(text, (i - 0.2, est[i + 1] + cis_size / 20), color=color_tm, size=15, weight='bold')

    ax.grid(which='both', axis='y', linestyle='')
    ax.grid(which='major', axis='x', linestyle='')
    ax.set_frame_on(False)
    custom_lines = [Line2D([0], [0], color=color_f1, lw=5),
                    Line2D([0], [0], color=color_tm, lw=5, linestyle='--')]
    ax.legend(custom_lines, ['average f1 improvement', 'average time increase'], fontsize='x-large', shadow=True,
              bbox_to_anchor=(0.35, 1.05))
    ax.legend()
    plt.title(model, size=18)
    fig.tight_layout()
    plt.show()
