...
from dbn.models import AbstractSupervisedDBN as BaseAbstractSupervisedDBN
from dbn.models import BaseModel
from dbn.models import BinaryRBM as BaseBinaryRBM
from dbn.models import UnsupervisedDBN as BaseUnsupervisedDBN
from dbn.utils import batch_generator, to_categorical
...
class BinaryRBM(BaseBinaryRBM, BaseTensorFlowModel):
    ...
    def _build_model(self, weights=None):
        ...
        sample_hidden_units_op = tf.dtypes.cast(self.random_uniform_values < self.compute_hidden_units_op, 'float32')
        ...
        for t in range(self.contrastive_divergence_iter):
            ...
            sample_hidden_units_gibbs_step_op = tf.dtypes.cast(
                random_uniform_values < compute_hidden_units_gibbs_step_op, 'float32')
    ...
    def _stochastic_gradient_descent(self, _data):
        ...
        for iteration in range(1, self.n_epochs + 1):
            ...
            data = _data[idx]
            generator = batch_generator(self.batch_size, data)
            n_batches = int(np.ceil(len(data) / float(self.batch_size)))
            for batch in tqdm(generator, total=n_batches, leave=False, file=sys.stdout, ncols=70):
            ...
            if self.verbose:
                ...
                print(f'{iteration:03d} error: {error:8.5f}')

            if error <= best_error:
                best_error = error
                best_epoch = iteration
            if self.patience > 0 and error > best_error and iteration >= best_epoch + self.patience:
                print(">> Finished training on Epoch %d" % iteration)
                break
...
class TensorFlowAbstractSupervisedDBN(BaseAbstractSupervisedDBN, BaseTensorFlowModel):
    ...
    def _fine_tuning(self, data, _labels):
        ...
        n_epochs = self._stochastic_gradient_descent(data, labels)
        if self.verbose:
    ...
    def _stochastic_gradient_descent(self, data, labels):
        ...
        for iteration in range(self.n_iter_backprop):
            generator = batch_generator(self.batch_size, data, labels)
            n_batches = int(np.ceil(len(data) / float(self.batch_size)))
            for batch_data, batch_labels in tqdm(generator, total=n_batches, leave=False, file=sys.stdout, ncols=70):
